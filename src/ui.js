...
function updateDisplay(rateEntries) {
  ...
  rateEntries.forEach(([currency, value]) => {
    ...
    div.innerHTML = `
      ...
      <div>
        <div class="description">${value.description || 'Currency exchange rate'}</div>
        ${value.region !== 'Unknown' ? `<div class="region">${value.region}</div>` : ''}
      </div>
...
function setupFilter() {
  ...
  // Create letter filter group
  const letterGroup = document.createElement('optgroup');
  letterGroup.label = "Filter by First Letter";
  
  // Collect and sort all first letters from currencies
  const letters = new Set();
  Object.keys(allRates).forEach(currency => {
    letters.add(currency.charAt(0).toUpperCase());
  });
  ...
  sortedLetters.forEach(letter => {
    ...
    letterGroup.appendChild(option);
  });
  
  filterSelect.appendChild(letterGroup);
